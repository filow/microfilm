...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if(C('URL_REWRITE_SWITCH')){
        $scriptfile=preg_replace('/\/index.php$/', '', __APP__);
    }else{  
        $scriptfile=__APP__;
    }
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        $url        =   $scriptfile.'?'.http_build_query(array_reverse($var));
        if(!empty($vars)) {
        ...
        if(isset($route)) {
            $url    =   $scriptfile.'/'.rtrim($url,$depr);
        }else{
            $url    =   $scriptfile.'/'.implode($depr,array_reverse($var));
        }
        if(!empty($vars)) { // 添加参数
